...
def make_network(name: str) -> NetworkClass:
    ...
    if name == 'Manchester_distribution_network_Kearsley':
    # elif name == 'Manchester_distribution_network_Kearsley':
        ...
        # 2.1) Old model - hardening cost proportional to line's length and amount of fragility curve shift
        ncon.data.net.hrdn_cost_rate_old = [1e4] * len(ncon.data.net.bch)  # £10k per km length and per m/s hardening

        # 2.2) New model - fixed fragility curve shift and hence fixed hardening cost per line
        ...
        # Hardening cost rate
        ncon.data.net.hrdn_cost_rate = [1e5] * len(ncon.data.net.bch)  # £100k/km for DN OHLs
    ...
    #     ncon.data.bch_hrdn_limits = [0.0, 30.0]  # in m/s (used in the old version investment model)
    ...
    #     return net
        ...
        # 2) Line hardening cost:
        # 2.1) Old model - hardening cost proportional to line's length and amount of fragility curve shift
        ncon.data.net.hrdn_cost_rate_old = [5e4] * len(ncon.data.net.bch)  # £50k per km length and per m/s hardening

        # 2.2) New model - fixed fragility curve shift and hence fixed hardening cost per line
        # (line hardening is modelled as a fixed fragility rightward shift, with cost proportional to the line length)
        ncon.data.net.fixed_hrdn_shift = [15.0] * len(ncon.data.net.bch)
        # Hardening cost rate
        ncon.data.net.hrdn_cost_rate = [5e5] * len(ncon.data.net.bch)  # £500k/km for TN lines
        # Compute hardening cost
        ncon.data.net.cost_bch_hrdn_fixed = []
        for i, length in enumerate(ncon.data.net.bch_length_km):
            is_line = ncon.data.net.bch_type[i] == 1  # 1=line, 0=transformer
        ...
        # Old model parameter (cost per km per m/s)
        ncon.data.net.hrdn_cost_rate_old.extend(dn.hrdn_cost_rate_old)
        ...
        # Old model parameter
        ncon.data.net.hrdn_cost_rate_old.insert(idx_cpl, 1e9)
