...
candidate_seeds = [112, 152, 166, 198]
...
for i, (file_path, seed) in enumerate(zip(generated_files, candidate_seeds), 1):
    # Generate a custom title for each scenario
    custom_title = f"Windstorm Path - Scenario {i} (Seed {seed})"

    visualize_windstorm_event(
        file_path=file_path,
        scenario_number=1,  # Since you're generating single scenarios
        event_number=1,  # Single event per scenario
